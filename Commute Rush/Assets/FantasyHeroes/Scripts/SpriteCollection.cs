...
namespace Assets.FantasyHeroes.Scripts
{
    ...
    public class SpriteCollection : MonoBehaviour
    {
        ...
        public void Refresh()
        {
            ...
            Cloak = LoadSprites(path + "/Equipment/Cloak");
        ...
        {
            return Directory.GetFiles(path, "*.png", SearchOption.AllDirectories).Select(i => UnityEditor.AssetDatabase.LoadAssetAtPath(i, typeof(Texture2D))).Cast<Texture2D>().OrderBy(i => i.name).ToList();
        }
